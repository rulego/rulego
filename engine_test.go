...
func TestOnDebug(t *testing.T) {
	...
	t.Run("hasOnDebug", func(t *testing.T) {
		...
		onDebugWg.Add(8)
		ruleEngine.OnMsg(msg, types.WithRunSnapshot(func(snapshot types.RuleChainRunSnapshot) {
			...
			snapshotWg.Done()
		}), types.WithRunOnDebug(func(ruleChainId string, flowType string, nodeId string, msg types.RuleMsg, relationType string, err error) {
			if nodeId == "s1" && flowType == types.Out {
				assert.Equal(t, types.True, relationType)
			}
			if nodeId == "s2" && flowType == types.Out {
