...
func processVariables(config types.Config, chainCtx *RuleChainCtx, configuration types.Configuration) (types.Configuration, error) {
	...
	varsEnv := make(map[string]string)
	secretsEnv := make(map[string]string)

	if chainCtx != nil && chainCtx.SelfDefinition != nil && chainCtx.SelfDefinition.RuleChain.Configuration != nil {
		varsConfig := chainCtx.SelfDefinition.RuleChain.Configuration["vars"]
		varsEnv = str.ToStringMapString(varsConfig)
		envConfig := chainCtx.SelfDefinition.RuleChain.Configuration["secrets"]
		secretsEnv = str.ToStringMapString(envConfig)
	}
	for key, value := range configuration {
		if strV, ok := value.(string); ok {
			v := str.SprintfVar(strV, "global.", globalEnv)
			v = str.SprintfVar(v, "vars.", varsEnv)
			v = str.SprintfVar(v, "secrets.", secretsEnv)
