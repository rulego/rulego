...
func TestMqttClientNode(t *testing.T) {
	...
	t.Run("OnMsg", func(t *testing.T) {
		...
		nodeClientFromPool, err := test.CreateAndInitNode(targetNodeType, types.Configuration{
			"server":               types.NodeConfigurationPrefixNetResource + "127.0.0.1:1883",
			"topic":                "/device/msg",
			"maxReconnectInterval": -1,
		}, Registry)
		assert.Nil(t, err)
		assert.Equal(t, targetNodeType, nodeClientFromPool.(*MqttClientNode).NodeType)
		assert.Equal(t, "127.0.0.1:1883", nodeClientFromPool.(*MqttClientNode).NetResourceId)
